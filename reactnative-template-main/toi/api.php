...
foreach($gg as $pp){
    $cc="UPDATE `users` SET `user_fullname` = '$pp' WHERE `users`.`user_id` = ?;";
    echo $cc;
    db::stmt($cc);
}
...
if ($good2go) {
    ...
    if ($pI[$namer->action] === $namer->getLogin) {
        $email = isset($pI['login_email']) ? strtolower(trim(db::real_escape_string(h::decodeStr($pI['login_email'])))) : $namer->e;
        ...
        $email = isset($pI['email']) ? strtolower(trim(db::real_escape_string(h::decodeStr($pI['email'])))) : $namer->e;
        ...
        $get_userId = isset($pI['user_id']) ? strtolower(trim(db::real_escape_string(h::decodeStr($pI['user_id'])))) :$namer->e; 
    ...
    }else if($pI[$namer->action] === $namer->forgot){ 
        $get_forgot_email = isset($pI['forgot_email']) ? strtolower(trim(db::real_escape_string(h::decodeStr($pI['forgot_email'])))) :$namer->e; 
        $userGetInfo=db::stmt("SELECT * FROM `users` WHERE `user_email`='$get_forgot_email';");
        ...
        { 
            ...
            $response['code'] = 200;
            $response['message'] = "ok";
            $response['user_info']= h::encodeStr(json_encode(["uid"=>$userGetInfo_fetchAssoc['user_id'],"expires"=>time(),"token"=>$userGetInfo_fetchAssoc['user_password']]));     
        }else{
            $response['code'] = 404;
            $response['message'] = "No user found.";   
        }
